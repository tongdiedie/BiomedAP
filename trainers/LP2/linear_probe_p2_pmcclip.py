...
class LinearProbe_P2_PMCCLIP(FSCLIPmethod):
    '''
    ...
    def forward(self,
        ...
        classifier = nn.Linear(features.shape[1], int(features.shape[0]/self.shot),bias=True).to(torch.float32).cuda()
        ...
        alpha_vec = Variable(final_init_alpha_mean * torch.ones(1, int(features.shape[0]/self.shot)).to(torch.float32).cuda(), requires_grad=True)
        ...
        for epoch in range(self.epoch):
            ...
            logits = vision_logits + torch.ones(features.shape[0],1).to(torch.float32).cuda() @ alpha_vec * text_logits
            ...
            logits_val = vision_logits_val + torch.ones(val_features.shape[0], 1).to(torch.float32).cuda() @ alpha_vec * text_logits_val
            ...
            if acc_val >= best_acc:
                ...
                logits_test = vision_logits_test + torch.ones(test_features.shape[0], 1).to(torch.float32).cuda() @ alpha_vec * text_logits_test
