# biomed_contrast_pubmedclip.py
"""
BiomedDPT_Contrast_PubMedCLIP
=============================
BiomedDPT with Contrastive Learning on PubMedCLIP backbone

核心改进：
1. 使用 PubMedCLIP (ViT-B/32) 作为 backbone
2. 负样本排斥策略：通过替换类别名称生成负样本
3. 4个损失函数：CE + L1_high + KL + Repulsion
"""
...
class BiomedDPT_Contrast_PubMedCLIP(TrainerX):
    """
    BiomedDPT with Contrastive Learning on PubMedCLIP backbone
    
    特点:
    - 使用 PubMedCLIP (ViT-B/32) 作为 backbone
    - 负样本排斥策略: 通过替换类别名称生成负样本
    - Visual Prompt: 在 ViT 的12层注入可学习的 prompt
    - 4个损失函数: CE + L1 + KL + Repulsion
    """
