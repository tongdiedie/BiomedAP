...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, biomedclip_model):
        ...
        biomedclip_model.cuda()
      
        temp = CUSTOM_TEMPLATES[cfg.DATASET.NAME]
        prompts_ = [temp.format(c.replace("_", " ")) for c in classnames]
        print(f"Prompts: {prompts_}")
        prompts_ = torch.cat([self.tokenizer(p) for p in prompts_])
        prompts_ = prompts_.cuda()


        with torch.no_grad():
            text_features = biomedclip_model.encode_text(prompts_)
        ...
        with torch.no_grad():
            embedding = biomedclip_model.text.transformer.embeddings.word_embeddings(tokenized_prompts.cuda()).type(dtype)
...
class KgCoOp_BiomedCLIP(TrainerX):
